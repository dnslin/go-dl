...
func worker(id int, taskChan <-chan DownloadTask, resultChan chan<- DownloadResult) error {
	...
	for task := range taskChan {
		...
		for retry := 0; retry < maxRetries; retry++ {
			log.Printf("工作协程 %d 开始处理任务 (ID: %s, 重试: %d)", id, task.ID, retry)
			err := downloadFile(task)
			if err == nil {
				result.Status = 1 // 下载成功
				log.Printf("工作协程 %d 成功完成任务 (ID: %s)", id, task.ID)
				break
			}

			log.Printf("工作协程 %d 下载失败 (ID: %s, 重试: %d): %v", id, task.ID, retry+1, err)
			result.Err = err

			if retry == maxRetries-1 {
				result.Status = 2 // 下载失败
				log.Printf("工作协程 %d 任务失败 (ID: %s)，达到最大重试次数", id, task.ID)
			}

			retryDelay := time.Second * time.Duration(retry+1)
			log.Printf("工作协程 %d 等待 %v 后重试任务 (ID: %s)", id, retryDelay, task.ID)
